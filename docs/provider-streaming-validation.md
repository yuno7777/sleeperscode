...
pressure while a consumer drains a stream larger than that capacity. They do not prove that the
queue actually became full. The cause of the earlier ad hoc timeout is unknown.

## Stalled-consumer behavior

The lossless probes above attach their consumers before each turn. A paired Node and Rust regression
now forks a 1,024 x 4 KiB turn, waits for a raw incoming-protocol receipt, and only then attaches the
event consumer. Both transports still deliver all 1,024 ordered deltas byte-for-byte.

This is a late-consumer regression, not proof of transport backpressure. Raw protocol receipts are
chunks of bytes and include protocol traffic; their count does not measure queued application events.
The previous assertion after 16 raw receipts therefore did not establish that the event queue was
full or that the agent had stalled. The 256-event capacity and the read-ahead ceiling remain
unvalidated under a consumer that stays stopped.
...
- Add a deterministic receipt at the event-queue boundary, then measure the byte ceiling of
  transport read-ahead and process-tree memory while a consumer stays stopped. The current
  regression proves only that attaching a consumer late loses nothing; the 256-event capacity
  remains an unexercised bound rather than a tuned value.
