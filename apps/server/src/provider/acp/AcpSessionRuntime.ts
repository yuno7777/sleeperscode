...
export interface AcpSessionRuntimeOptions {
  ...
  /**
   * Authentication method selected by a provider-specific integration.
   * Generic ACP agents may manage credentials themselves and omit this;
   * in that case the client initializes the protocol without inventing an
   * authentication method the agent did not advertise.
   */
...
export class AcpSessionRuntime extends Context.Service<
  AcpSessionRuntime,
  {
    ...
    /** Current bounded event-queue depth, exposed for diagnostics and saturation tests. */
    readonly getEventQueueDepth: Effect.Effect<number>;
